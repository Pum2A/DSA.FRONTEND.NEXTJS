...
export default function ModulesPage() {
  const { data, isLoading } = useQuery<{ modules: ModuleDto[]; totalModules: number }>({
    queryKey: ["allModules"],
    queryFn: async () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.length > 0 ? modules.map((module) => {
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-between text-white">
                    <div>
                      <Badge 
                        ...
                        className={isLocked ? "border-white/40 text-white/90" : "bg-white/20"}
                      >DSA</Badge>
                    </div>
                    <div>
            ...
            <div className="col-span-full text-center py-12">
              ...
              <p className="text-muted-foreground mt-2">Nowe moduły będą dostępne wkrótce.</p>
            </div>
          )}
        </div>
      </div>
